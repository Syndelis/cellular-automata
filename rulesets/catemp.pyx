...
cdef class CA:
    ...
    cpdef void addrandomvalues(self, value: int=0, state: int=0, N: int=10, p: float=1):
        ...
        N: An integer represeting the number of attempts made.
        p: A float between 0 and 1 represeting the probability of assignment the value where cells has the state passed
        as argument.
...
cpdef list neighbors8(obj, x, y, old=False):
    ...
    cpdef list events(obj, odds=1.):
        if isinstance(obj, PCA):
            return obj.__events__(odds)
        else: raise TypeError("Object `obj` must be an instance/subclass of CA")

    cpdef void __savetxt__(self, filename) except *:
        fd = open(filename, "w")
        cdef int i, j
        for i in range(0, self.domain_size):
            for j in range(0, self.domain_size):
                fd.write(str(self.domain[i][j]) + " ")
            fd.write("\n")
        fd.close()

    cpdef void savetxt(obj, filename):
        if isinstance(obj, PCA):
            obj.__savetxt__(filename)
